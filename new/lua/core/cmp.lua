...
local function jumpable(dir)
    ...
    local function seek_luasnip_cursor_node()
        ...
        if exit_node then
            ...
            if (pos[1] > exit_pos_end[1]) or
                (pos[1] == exit_pos_end[1] and pos[2] > exit_pos_end[2]) then
        ...
        while node ~= nil and node.next ~= nil and node ~= snippet do
            ...
            local candidate = n_next ~= snippet and next_pos and
                                  (pos[1] < next_pos[1]) or
                                  (pos[1] == next_pos[1] and pos[2] <
                                      next_pos[2])
    ...
    if dir == -1 then
        ...
        return luasnip.in_snippet() and seek_luasnip_cursor_node() and
                   luasnip.jumpable(1)
    end
end
    ...
    cmp = {
        ...
        formatting = {
            ...
            format = function(entry, vim_item)
                ...
                vim_item.dup = cmp.formatting.duplicates[entry.source.name] or
                                   cmp.formatting.duplicates_default
                return vim_item
            end
        },
        ...
        sources = {
            ...
            }, {name = "path"}, {name = "luasnip"}, {name = "cmp_tabnine"},
            {name = "nvim_lua"}, {name = "buffer"}, {name = "calc"},
            {name = "emoji"}, {name = "treesitter"}, {name = "crates"},
            {name = "tmux"}
        },
        mapping = cmp_mapping.preset.insert {
            ["<C-k>"] = cmp_mapping(cmp_mapping.select_prev_item(), {"i", "c"}),
            ["<C-j>"] = cmp_mapping(cmp_mapping.select_next_item(), {"i", "c"}),
            ["<Down>"] = cmp_mapping(cmp_mapping.select_next_item {
                behavior = SelectBehavior.Select
            }, {"i"}),
            ["<Up>"] = cmp_mapping(cmp_mapping.select_prev_item {
                behavior = SelectBehavior.Select
            }, {"i"}),
