...
M.toggle_log_view = function(logfile)
    local log_viewer = bvim.log.viewer.cmd
    if vim.fn.executable(log_viewer) ~= 1 then log_viewer = "less +F" end
    log_viewer = log_viewer .. " " .. logfile
    local term_opts = vim.tbl_deep_extend("force", terminal, {
        cmd = log_viewer,
        open_mapping = bvim.log.viewer.layout_config.open_mapping,
        direction = bvim.log.viewer.layout_config.direction,
        -- TODO: this might not be working as expected
        size = bvim.log.viewer.layout_config.size,
        float_opts = bvim.log.viewer.layout_config.float_opts
    })
