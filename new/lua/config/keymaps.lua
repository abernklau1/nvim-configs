...
local defaults = {
    ...
    normal_mode = {
        ...
        -- C++ Compile Command
        ["<leader>a"] = "<cmd>make build<CR><cmd>./%<<CR>",

        -- LSP Diagnostics
        ["<leader>d"] = "<cmd>lua vim.diagnostic.open_float({bufnr=0})<CR>",

        -- Bufferline
        ["<leader>s"] = "<cmd>BufferLinePick<CR>",
        ["<leader>b"] = "<cmd>BufferLineCyclePrev<CR>",
        ["<leader>n"] = "<cmd>BufferLineCycleNext<CR>",
        ["<leader>c"] = "<cmd>BufferLinePickClose<cr>",
...
function M.clear(keymaps)
    ...
    for mode, mappings in pairs(keymaps) do
        local translated_mode = mode_adapters[mode] and mode_adapters[mode] or
                                    mode
        for key, _ in pairs(mappings) do
            -- some plugins may override default bindings that the user hasn't manually overriden
            if default[mode][key] ~= nil or
                (default[translated_mode] ~= nil and
                    default[translated_mode][key] ~= nil) then
...
function M.load(keymaps)
    keymaps = keymaps or {}
    for mode, mapping in pairs(keymaps) do M.load_mode(mode, mapping) end
end
...
function M.load_defaults()
    ...
    local keys = {}
    for idx, _ in pairs(defaults) do if not keys[idx] then keys[idx] = {} end end
end
